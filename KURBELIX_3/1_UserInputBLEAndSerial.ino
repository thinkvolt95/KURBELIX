...
// Power Vector Activation / Deactivation

else if (cmd.startsWith("v")) {

  // Nur 'v' → Status anzeigen
  if (cmd.length() == 1) {
    logPrint("Power Vector ist aktuell: ");
    logPrintln(enablePowerVector ? "ENABLED" : "DISABLED");
    return;
  }
  ...
  if (mode == '1') {
    enablePowerVector = true;
    logPrintln("Power Vector ENABLED");
  }
  else if (mode == '0') {
    enablePowerVector = false;
    logPrintln("Power Vector DISABLED");
  }
  else {
    logPrintln("Usage: v  |  v1 (enable)  |  v0 (disable)");
  }
}
