/*
  ...
  - Added Cycling Power Vector
  - Added Low Pass Filtering for Detect Revolution
...
// --- Global variables for crank angle calculation
float currentCrankAngle = 0.0f;

// -- variables for Cycling Power Vector
#define MAX_VECTOR_SAMPLES 6  // Max Samples for 23-Byte MTU
int16_t torqueVector[MAX_VECTOR_SAMPLES];
uint8_t vectorCount = 0;
uint16_t firstSampleAngle = 0;  // Angle of the first sample
  ...
  if (hx711ReadyFlag) {
    ...
    if (isfinite(scaleFactor_counts_per_N) && fabs(scaleFactor_counts_per_N) > 1e-6f) {
      ...
      if (shouldAccumulateTorqueSample(torqueNm, correctedGyroZ_dps)) {
        ...
        // If first sample of new revolution, save the start angle
        extern uint8_t vectorCount;
        extern uint16_t firstSampleAngle;
        extern float currentCrankAngle;
        if (vectorCount == 0) {
          firstSampleAngle = (uint16_t)currentCrankAngle;
        }

        // Save Samples in vector buffer
        recordVectorSample(torqueNm);
      }
    }
    lastGyroZ_dps = rawGyroZ_dps;
    float degZ = rawGyroZ_dps;
    if (detectRevolution(sampleMicros, degZ)) {
      extern int16_t torqueVector[];
      extern uint8_t vectorCount;
      extern uint16_t firstSampleAngle;

      //Send cycling power vector to garmin
      sendCyclingPowerVector(torqueVector, vectorCount, firstSampleAngle);
      // Calculate power & cadence
      CadencePowerCalc(revolutionTimestampUs);

    } else {
      //Function for coasting / standstill check of crank
      checkStandstill();
    }
  }
