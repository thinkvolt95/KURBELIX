...
void accumulateTorque(float torqueNm, float correctedGyroZ_dps) {
  // Convert angular velocity from degrees per second (dps) to radians per second (rad/s)
  float angVelRad = correctedGyroZ_dps * (PI / 180.0f);
  float w = fabsf(angVelRad);

  // 1) Dead centers & very slow movement: assign minimal weight
  if (w < 0.5f) {
    w = 0.1f;  // Minimal contribution
  }

  // 2) Upper limit: prevent high-speed segments from dominating the average
  if (w > 10.0f) {
    w = 10.0f;
  }

  // Weighted accumulation of torque and sample count
  sumTorqueNm      += torqueNm * w;
  torqueSampleCount += w;
}
...
void runCalibration(float knownMassKg) {
  ...
  logPrint("loadedAvg = ");
  logPrintln(String(loadedAvg));
  logPrint("zeroOffsetCounts = ");
  logPrintln(String(zeroOffsetCounts));
  logPrint("deltaCounts = ");
  logPrintln(String(deltaCounts));
  logPrint("forceN = ");
  logPrintln(String(forceN, 6));
  logPrint("scaleFactor_counts_per_N = ");
  logPrintln(String(scaleFactor_counts_per_N, 6));
